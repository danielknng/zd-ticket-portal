...
import { ZAMMAD_API_URL } from './nf-dom.js';
...
class ZammadApiClient {
    ...
    async authenticate(username, password) {
        ...
        const response = await nfApiGet(this._buildUrl('/users/me'), {
            headers: {
    ...
    async getTicket(ticketId) {
        ...
        const ticketResponse = await nfApiGet(this._buildUrl(`/tickets/${ticketId}`), {
            headers: this._getHeaders()
        });
        ...
        const articlesResponse = await nfApiGet(this._buildUrl(`/ticket_articles/by_ticket/${ticketId}`), {
            headers: this._getHeaders()
        });
    ...
    async getTickets(filters = {}) {
        ...
        const response = await nfApiGet(this._buildUrl(`/tickets/search?query=${encodeURIComponent(query)}`), {
            headers: this._getHeaders()
        });
    ...
    async createTicket(ticketData) {
        ...
        if (requestType && window.NF_CONFIG?.api?.allowRequestType) {
            payload.type = requestType;
        }

        const response = await nfApiPost(this._buildUrl('/tickets'), payload, {
            headers: this._getHeaders()
        });
    ...
    async sendReply(ticketId, text, files) {
        ...
        const response = await nfApiPost(this._buildUrl('/ticket_articles'), articleData, {
            headers: this._getHeaders()
        });
        ...
        if (files && files.length > 0) {
            for (const file of files) {
                ...
                await nfApiPost(this._buildUrl('/ticket_attachment'), attachmentData, {
                    headers: this._getHeaders()
                });
            }
        }
    ...
    async closeTicket(ticketId) {
        ...
        const response = await nfApiPut(this._buildUrl(`/tickets/${ticketId}`), { state_id: 4 }, {
            headers: this._getHeaders()
        });
    ...
    async getRequestTypes() {
        const response = await nfApiGet(this._buildUrl('/object_manager_attributes?object=Ticket&name=type'), {
            headers: this._getHeaders()
        });
 ...
 * @param {string} [baseUrl] - Base URL (defaults to ZAMMAD_API_URL())
...
export function createApiClient(baseUrl, authToken = null) {
    const url = baseUrl || ZAMMAD_API_URL();
    if (!url) {
